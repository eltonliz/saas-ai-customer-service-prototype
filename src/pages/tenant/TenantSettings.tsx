...
export default function TenantSettings({ context }: PageProps) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-900 mb-4">租户设置</h2>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Settings size={16} className="text-blue-500" />基础信息</h3>
            <button type="button" onClick={openBasicEdit} className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50">
              <Edit3 size={12} />编辑
            </button>
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2"><Zap size={16} className="text-amber-500" />套餐额度</h3>
          <div className="space-y-3 text-sm">
            <div>
              <div className="flex justify-between mb-1"><span className="text-slate-500">Token额度</span><span className="text-sm text-slate-400">{tenant?.tokenUsed?.toLocaleString()} / {tenant?.tokenLimit?.toLocaleString()}</span></div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Key size={16} className="text-violet-500" />渠道配置</h3>
            <button type="button" onClick={openChannelsEdit} className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50">
              <Edit3 size={12} />编辑
            </button>
          </div>
          <div className="flex flex-wrap gap-2">
            {tenant?.channels.map((c) => (
              <span key={c} className="rounded-lg bg-violet-50 px-3 py-1.5 text-sm font-medium text-violet-700 border border-violet-100">{c}</span>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Shield size={16} className="text-indigo-500" />角色权限</h3>
            <button type="button" onClick={openRolesEdit} className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50">
              <Edit3 size={12} />编辑
            </button>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-slate-500">租户管理员</span><span className="text-sm text-slate-400">完全访问</span></div>
            <div className="flex justify-between"><span className="text-slate-500">客服主管</span><span className="text-sm text-slate-400">管理客服、查看数据</span></div>
            <div className="flex justify-between"><span className="text-slate-500">客服人员</span><span className="text-sm text-slate-400">工作台、会话、工单</span></div>
            <div className="flex justify-between"><span className="text-slate-500">门店人员</span><span className="text-sm text-slate-400">本门店会话、核销</span></div>
          </div>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-5 col-span-1 lg:col-span-2">
          <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2"><FileText size={16} className="text-slate-500" />操作日志</h3>
      ...
      <Modal open={editModal === "basic"} title="编辑基础信息" onClose={() => setEditModal(null)} size="md">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditModal(null)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleBasicSave} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>

      {/* Channel Edit Modal */}
      <Modal open={editModal === "channels"} title="编辑渠道配置" onClose={() => setEditModal(null)} size="md">
        <p className="text-sm text-slate-500 mb-3">选择已开通的渠道</p>
        <div className="flex flex-wrap gap-2">
          {allChannels.map((ch) => (
            <label
              key={ch}
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditModal(null)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleChannelsSave} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
      ...
      <Modal open={editModal === "roles"} title="编辑角色权限" onClose={() => setEditModal(null)} size="md">
        <div className="space-y-3">
          {editRoles.map((role, idx) => (
            <div key={role.role}>
              <label className="block text-sm font-medium text-slate-500 mb-1">{role.role}</label>
              <input
                value={role.desc}
                onChange={(e) => {
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditModal(null)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleRolesSave} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
    </div>
  );
