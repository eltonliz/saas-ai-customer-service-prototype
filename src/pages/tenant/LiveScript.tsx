    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">直播话术管理</h2>
            <p className="text-sm text-slate-500 mt-1">管理直播带货场景下的AI客服推荐话术和标准应答</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">话术标题</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">业务线</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">关联商品</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">字数</th>
          ...
          <tbody>
            {mockScripts.map((s) => (
              <tr key={s.id} className="border-b border-slate-100 hover:bg-slate-50" style={{ minHeight: 48 }}>
                <td className="px-5 py-3 text-sm font-medium text-slate-800">{s.title}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{s.businessLine}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{s.product}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{s.words}</td>
                <td className="px-5 py-3">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${s.status === "已启用" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{s.status}</span>
                </td>
                <td className="px-5 py-3">
                  <button className="text-sm text-blue-600 hover:text-blue-800 mr-3">编辑</button>
                  <button className="text-sm text-slate-400 hover:text-slate-600 mr-3">预览</button>
                  {s.status === "已启用" ? (
