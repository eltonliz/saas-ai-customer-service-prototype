    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">售后政策管理</h2>
            <p className="text-sm text-slate-500 mt-1">配置和管理各业务线的售后、退款、退换货规则</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">政策标题</th>
          ...
          <tbody>
            {mockPolicies.map((p) => (
              <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50" style={{ minHeight: 48 }}>
                <td className="px-5 py-3 text-sm font-medium text-slate-800">{p.title}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{p.scope}</td>
                ...
                <td className="px-5 py-3">
                  ...
                  <button className="text-sm text-slate-400 hover:text-slate-600 mr-3">详情</button>
                  {p.status === "已启用" ? (
