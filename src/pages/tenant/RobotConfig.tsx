  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-slate-900">AI机器人配置</h2>
        <button type="button" onClick={openNewModal} className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"><Plus size={14} />新增机器人</button>
      </div>
        ...
        {robots.map((r) => (
          <div key={r.id} className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <div className="flex gap-2 mt-4 pt-3 border-t border-slate-100">
              <button type="button" onClick={() => toggleRobot(r.id)} className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium ${
              ...
              <button type="button" onClick={handleTest} className="flex items-center gap-1 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-600"><Play size={12} />测试</button>
              <button type="button" onClick={() => openEditModal(r)} className="flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-sm font-medium text-amber-600"><Edit3 size={12} />编辑</button>
              <button type="button" onClick={() => openViewModal(r.id)} className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-slate-600"><Eye size={12} />详情</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <Modal open={modal === "new"} title="新增机器人" onClose={() => setModal(null)} size="lg">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">机器人名称</label>
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="输入名称" className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-400" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">绑定渠道</label>
            <select value={form.channels} onChange={(e) => setForm((f) => ({ ...f, channels: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none">
              {["APP","小程序","H5","商家后台","企业微信","公众号/微信客服"].map((c) => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-500 mb-1">欢迎语</label>
            <input value={form.welcome} onChange={(e) => setForm((f) => ({ ...f, welcome: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">回答风格</label>
            <select value={form.style} onChange={(e) => setForm((f) => ({ ...f, style: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none">
              {["专业友好","简洁高效","亲切温和","专业严谨"].map((s) => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">模型</label>
            <select value={form.model} onChange={(e) => setForm((f) => ({ ...f, model: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none">
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button type="button" onClick={handleCreate} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">创建</button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "edit"} title="编辑机器人" onClose={() => { setModal(null); setEditingRobotId(null); }} size="lg">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => { setModal(null); setEditingRobotId(null); }} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleEdit} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "view"} title="机器人详情" onClose={() => { setModal(null); setViewingRobotId(null); }} size="lg">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => { setModal(null); setViewingRobotId(null); }} className="rounded-lg border px-4 py-2 text-sm">关闭</button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "test"} title="测试机器人" onClose={() => setModal(null)} size="md">
        <div className="h-64 overflow-y-auto rounded-xl bg-slate-50 p-3 mb-3">
          {testMessages.map((m, i) => (
            <p key={i} className={`text-sm mb-2 ${m.startsWith("用户:") ? "text-blue-600" : "text-slate-600"}`}>{m}</p>
          ))}
          {testMessages.length === 0 && <p className="text-sm text-slate-400 text-center pt-20">输入测试消息</p>}
        </div>
        <div className="flex gap-2">
          <input value={testInput} onChange={(e) => setTestInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendTestMsg()} placeholder="输入测试问题..." className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none" />
          <button type="button" onClick={sendTestMsg} className="rounded-xl bg-blue-600 px-4 py-2 text-sm text-white">发送</button>
        </div>
      </Modal>
    </div>
  );
