    ...
    <div className="flex h-[calc(100vh-160px)] gap-0 overflow-hidden rounded-xl border border-slate-200 bg-white">
      ...
      <div className="w-[300px] shrink-0 border-r border-slate-200 flex flex-col bg-slate-50/30">
        <div className="p-4 border-b border-slate-200">
          <h3 className="text-sm font-semibold text-slate-700 mb-3">会话队列</h3>
          <div className="space-y-0.5">
            {groups.map((g) => {
              ...
              return (
                ...
                >
                  {g}
                  <span className="text-sm text-slate-400 bg-slate-200 rounded-full px-2 py-0.5 min-w-[24px] text-center">{count}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {filtered.map((c) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <p className="text-sm font-medium text-slate-700 truncate">{c.title.slice(0, 16)}</p>
          ...
          {filtered.length === 0 && (
            <p className="text-center text-sm text-slate-400 py-8">暂无会话</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3 bg-white">
          <div>
            <h3 className="text-sm font-semibold text-slate-800">{selected?.title ?? "选择会话"}</h3>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-sm text-slate-400">{selected?.intent}</span>
              <span className="text-sm text-slate-300">·</span>
              <span className="text-sm text-slate-400">{selected?.channel}</span>
              {selected?.riskLevel && <StatusBadge status={selected.riskLevel} />}
            </div>
          </div>
          <div className="flex gap-2">
            {activeGroup === "待接入" && (
              <button type="button" onClick={accessConversation} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">接入会话</button>
            )}
            <button type="button" onClick={() => setModal("工单")} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 flex items-center gap-1.5"><FilePlus size={15} />创建工单</button>
            <button type="button" onClick={() => setModal("转接")} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 flex items-center gap-1.5"><UserPlus size={15} />转接</button>
            <button type="button" onClick={() => setModal("结束")} className="rounded-lg border border-red-200 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 flex items-center gap-1.5"><XCircle size={15} />结束</button>
          </div>
        </div>
        ...
        <div className="border-t border-slate-200 p-4 bg-white">
          <div className="flex gap-2 mb-3 flex-wrap">
            {quickReplies.map((qr) => (
              <button key={qr} type="button" onClick={() => useQuickReply(qr)} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm text-slate-600 hover:bg-blue-50 hover:text-blue-700 hover:border-blue-200 transition-colors">{qr}</button>
            ))}
          </div>
          <div className="flex gap-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendReply()} placeholder="输入回复..." className="flex-1 rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-blue-400" />
            <button type="button" onClick={sendReply} className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-white hover:bg-blue-700"><Send size={18} /></button>
          </div>
        </div>
      </div>
      ...
      <div className="w-[380px] shrink-0 border-l border-slate-200 bg-white flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400">{user?.maskedPhone}</p>
                </div>
              </div>
              <div className="space-y-3">
                <div>
                  <span className="text-sm text-slate-400">会员等级</span>
                  <p className="text-sm font-medium text-slate-700">{user?.level}</p>
                </div>
                <div>
                  <span className="text-sm text-slate-400">来源渠道</span>
                  <p className="text-sm text-slate-700">{selected?.channel}</p>
                </div>
                <div>
                  <span className="text-sm text-slate-400">用户标签</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(user?.tags ?? []).map((t) => (
                      <span key={t} className="rounded-md bg-slate-100 px-2 py-0.5 text-sm text-slate-600">{t}</span>
                    ))}
                  </div>
                </div>
                <div>
                  <span className="text-sm text-slate-400">会话意图</span>
                  <p className="text-sm text-slate-700">{selected?.intent}</p>
                </div>
                <div>
          ...
          {rightTab === "orders" && (
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-slate-700">订单信息</h4>
              {currentOrder ? (
                <div className="rounded-xl border border-slate-200 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <ShoppingBag size={16} className="text-blue-500" />
                    <span className="text-sm font-medium text-slate-700">{currentOrder.productName}</span>
                  </div>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span className="text-slate-400">售后状态</span><span className="text-sm">{currentOrder.afterSaleStatus}</span></div>
                  </div>
                </div>
              ) : (
                <p className="text-sm text-slate-400 py-4 text-center">暂无订单信息</p>
              )}
              {currentOrder?.afterSaleStatus === "可申请" && (
                <button type="button" className="w-full rounded-lg border border-blue-200 bg-blue-50 px-4 py-2 text-sm text-blue-600 hover:bg-blue-100">申请售后</button>
              )}
            </div>
          )}
          ...
          {rightTab === "aiSuggestion" && (
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Bot size={16} className="text-indigo-500" />AI处理摘要</h4>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                <div>
                  <span className="text-sm text-slate-400">意图识别</span>
                  <p className="text-sm text-slate-700">商品库存咨询 · 置信度 0.91</p>
                </div>
                <div>
                  <span className="text-sm text-slate-400">工具调用</span>
                  <p className="text-sm text-slate-700">商品库存查询 · 调用成功</p>
                </div>
                <div>
                  <span className="text-sm text-slate-400">风控审核</span>
                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="w-2 h-2 rounded-full bg-emerald-500" />
                    <span className="text-sm text-emerald-700">通过 · 低风险</span>
                  </div>
                </div>
                <div>
                  <span className="text-sm text-slate-400">AI推荐回复</span>
                  <div className="mt-1 rounded-lg bg-white border border-slate-100 p-3 text-sm text-slate-700">
                    当前直播专享营养套装库存充足，售价199元。价格以直播间实时页面为准。
                    <div className="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100">
                      <Star size={12} className="text-amber-400" />
                      <span className="text-sm text-slate-400">置信度 0.91</span>
                    </div>
                  </div>
                  <button type="button" onClick={() => setInput("当前直播专享营养套装库存充足，售价199元。价格以直播间实时页面为准。")} className="mt-2 w-full rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-100">一键填入输入框</button>
                </div>
              </div>
              <button type="button" onClick={() => setTraceOpen(true)} className="w-full flex items-center justify-between rounded-lg border border-slate-200 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50">
          ...
          {rightTab === "knowledge" && (
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><BookOpen size={16} className="text-violet-500" />推荐知识</h4>
                ...
                {[
                  ...
                  <div key={k.title} className="rounded-lg border border-slate-100 p-3 hover:bg-slate-50 cursor-pointer">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm text-emerald-600">{k.match}</span>
                    </div>
                    <span className="text-sm text-slate-400">{k.type}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {rightTab === "records" && (
            <div className="space-y-5">
              <div>
                <h4 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2"><Tag size={14} className="text-blue-500" />会话标签</h4>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {convTags.map((t) => (
                    <span key={t} className="rounded-md bg-blue-50 px-2.5 py-1 text-sm text-blue-600">{t}</span>
                  ))}
                  {convTags.length === 0 && <span className="text-sm text-slate-400">暂无标签</span>}
                </div>
                <div className="flex gap-2">
                  <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} placeholder="添加标签" className="flex-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm outline-none focus:border-blue-400" />
                  <button type="button" onClick={addTag} className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white hover:bg-blue-700"><Tag size={14} /></button>
                </div>
              </div>

              <div>
                <h4 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2"><MessageSquare size={14} className="text-slate-500" />内部备注</h4>
                <div className="space-y-2 mb-3">
                  {convNotes.map((n, i) => (
                    <div key={i} className="text-sm text-slate-600 bg-slate-50 rounded-lg p-3">{n}</div>
                  ))}
                  {convNotes.length === 0 && <span className="text-sm text-slate-400">暂无备注</span>}
                </div>
                <div className="flex gap-2">
                  <input type="text" value={noteInput} onChange={(e) => setNoteInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addNote()} placeholder="添加备注..." className="flex-1 rounded-lg border border-slate-200 px-3 py-1.5 text-sm outline-none focus:border-blue-400" />
                  <button type="button" onClick={addNote} className="rounded-lg bg-slate-600 px-3 py-1.5 text-sm text-white hover:bg-slate-700"><MessageSquare size={14} /></button>
                </div>
              </div>

              {linkedTicket && (
                <div>
                  <h4 className="text-sm font-semibold text-slate-700 mb-2 flex items-center gap-2"><FileText size={14} className="text-violet-500" />关联工单</h4>
                  <div className="rounded-lg border border-slate-200 p-3">
                    <p className="text-sm font-medium text-slate-700">{linkedTicket.title}</p>
                    <div className="flex items-center gap-2 mt-1">
                      <StatusBadge status={linkedTicket.status} />
                      <span className="text-sm text-slate-400">{linkedTicket.owner}</span>
                    </div>
                  </div>
                </div>
              )}
          ...
          {rightTab === "trace" && (
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-slate-700 flex items-center gap-2"><Search size={16} className="text-indigo-500" />AI链路摘要</h4>
              <div className="rounded-xl border border-slate-200 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-emerald-500" />
                  <span className="text-sm text-slate-700">意图识别</span>
                  <span className="text-sm text-slate-400 ml-auto">商品库存咨询</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-blue-500" />
                  <span className="text-sm text-slate-700">工具调用</span>
                  <span className="text-sm text-slate-400 ml-auto">商品库存查询 · 成功</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-emerald-500" />
                  <span className="text-sm text-slate-700">风控审核</span>
      ...
      <Drawer open={traceOpen} title="AI链路详情" onClose={() => setTraceOpen(false)}>
        <div className="space-y-4 text-sm">
          <div className="rounded-xl border border-slate-200 p-4">
            <h4 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Bot size={14} className="text-indigo-500" />基础信息</h4>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-sm text-slate-400">意图</span><p>商品库存咨询</p></div>
              <div><span className="text-sm text-slate-400">置信度</span><p className="font-semibold">0.91</p></div>
              <div><span className="text-sm text-slate-400">模型</span><p>Claude Opus 4.7</p></div>
              <div><span className="text-sm text-slate-400">Prompt版本</span><p>v2.3</p></div>
              <div><span className="text-sm text-slate-400">Token消耗</span><p>620</p></div>
              <div><span className="text-sm text-slate-400">生成延迟</span><p>2,200ms</p></div>
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <h4 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Search size={14} className="text-blue-500" />工具调用详情</h4>
            <div className="space-y-2 text-sm">
              ...
              <div>
                <span className="text-sm text-slate-400 block mb-1">输入参数</span>
                <div className="rounded bg-slate-50 p-2 text-sm font-mono text-slate-600">查询商品: 直播专享营养套装</div>
              </div>
              <div>
                <span className="text-sm text-slate-400 block mb-1">返回结果</span>
                <div className="rounded bg-slate-50 p-2 text-sm font-mono text-slate-600">库存充足，实时库存数量已返回</div>
              </div>
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <h4 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Shield size={14} className="text-emerald-500" />风控审核详情</h4>
            <div className="space-y-2 text-sm">
      ...
      <Modal open={modal === "工单"} title="创建工单" onClose={() => setModal(null)} size="md">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">工单标题</label>
            <input type="text" value={ticketForm.title} onChange={(e) => setTicketForm((f) => ({ ...f, title: e.target.value }))} placeholder="请输入工单标题" className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-blue-400" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">工单类型</label>
            <select value={ticketForm.type} onChange={(e) => setTicketForm((f) => ({ ...f, type: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none">
              {["售后工单", "投诉工单", "门店工单", "课程工单", "大健康合规工单"].map((t) => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">优先级</label>
            <select value={ticketForm.priority} onChange={(e) => setTicketForm((f) => ({ ...f, priority: e.target.value as Priority }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none">
              {["低", "中", "高", "紧急"].map((p) => <option key={p}>{p}</option>)}
            </select>
          </div>
        </div>
        <div className="flex justify-end gap-3 mt-6">
          <button type="button" onClick={() => setModal(null)} className="rounded-lg border border-slate-200 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50">取消</button>
          <button type="button" onClick={createTicket} className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm text-white hover:bg-blue-700">创建工单</button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "转接"} title="转接会话" onClose={() => setModal(null)} size="sm">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">技能组</label>
            <select value={transferForm.skillGroup} onChange={(e) => setTransferForm((f) => ({ ...f, skillGroup: e.target.value, agent: "" }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none">
              {skillGroups.map((g) => <option key={g}>{g}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">转接客服</label>
            <select value={transferForm.agent} onChange={(e) => setTransferForm((f) => ({ ...f, agent: e.target.value }))} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none">
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button type="button" onClick={() => setModal(null)} className="rounded-lg border border-slate-200 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50">取消</button>
          <button type="button" onClick={transferConversation} className="rounded-lg bg-orange-500 px-4 py-2.5 text-sm text-white hover:bg-orange-600">确认转接</button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "结束"} title="结束会话" onClose={() => setModal(null)} size="sm">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">结束原因</label>
            <select value={endReason} onChange={(e) => setEndReason(e.target.value)} className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none">
        ...
        <div className="flex justify-end gap-3 mt-6">
          <button type="button" onClick={() => setModal(null)} className="rounded-lg border border-slate-200 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50">取消</button>
          <button type="button" onClick={endConversation} className="rounded-lg bg-red-500 px-4 py-2.5 text-sm text-white hover:bg-red-600">结束会话</button>
        </div>
      </Modal>
    </div>
  );
