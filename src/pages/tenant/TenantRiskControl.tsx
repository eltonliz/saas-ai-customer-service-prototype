    ...
    <div>
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="rounded-xl border border-slate-200 bg-white p-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <button type="button" onClick={() => openAddModal("sensitive")} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-base font-medium text-white hover:bg-blue-700">
                <Plus size={14} />添加敏感词
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {sensitiveWords.map((w) => (
                <span key={w} className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-3 py-1 text-base text-red-600 border border-red-100">
                  {w}
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <button type="button" onClick={() => openAddModal("banned")} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-base font-medium text-white hover:bg-blue-700">
                <Plus size={14} />添加禁用话术
              </button>
            </div>
            <div className="space-y-2">
              {bannedPhrases.map((p) => (
                <div key={p} className="flex items-center justify-between text-base text-red-600 bg-red-50 rounded-lg px-3 py-2">
                  <span>"{p}"</span>
      ...
      {tab === "templates" && (
        <div className="rounded-xl border border-slate-200 bg-white p-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <button type="button" onClick={() => openAddModal("template")} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-base font-medium text-white hover:bg-blue-700">
              <Plus size={14} />添加安全模板
            </button>
          </div>
          <div className="space-y-2">
      ...
      {tab === "records" && (
        <div className="space-y-3">
          <p className="text-base text-slate-500">高风险会话记录（本租户大健康风控样本）</p>
          {mySamples.map((s) => (
      ...
      {tab === "compliance" && (
        <div className="rounded-xl border border-slate-200 bg-white p-8">
          ...
          <p className="text-base text-slate-500">所有大健康相关会话将在风控审核后留痕。高风险会话自动转人工并记录合规审核结果。</p>
