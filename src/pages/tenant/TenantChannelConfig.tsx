    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">渠道配置</h2>
            <p className="text-sm text-slate-500 mt-1">管理各接入渠道的参数配置、开关状态及会话分配规则</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">日会话量</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">转化率</th>
            ...
            {channels.map((ch) => (
              <tr key={ch.id} className="border-b border-slate-100 hover:bg-slate-50" style={{ minHeight: 48 }}>
                ...
                <td className="px-5 py-3">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${ch.status === "已启用" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{ch.status}</span>
                </td>
                <td className="px-5 py-3 text-sm text-slate-600">{ch.dailySessions.toLocaleString()}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{ch.conversionRate}</td>
                <td className="px-5 py-3">
