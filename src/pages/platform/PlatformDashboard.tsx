...
import { Building2, Store, MessageCircle, Zap, Database, TrendingUp, AlertTriangle, Activity, Shield, Users, CreditCard } from "lucide-react";
...
export default function PlatformDashboard({}: PageProps) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-900 mb-6">平台总览</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {metrics.map((m) => (
          <div key={m.label} className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <span className={`inline-block text-sm font-medium px-2 py-0.5 rounded-full ${m.trendColor}`}>
              {m.trend}
            </span>
          </div>
        ))}
