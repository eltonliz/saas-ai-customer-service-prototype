  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-slate-900">商家管理</h2>
        <button
          type="button"
      ...
      <DataTable
        ...
        columns={[
          { key: "name", header: "商家名称", render: (r) => <span className="font-medium text-sm">{r.name}</span> },
          { key: "tenant", header: "所属租户", render: (r) => <span className="text-sm">{r.tenantId}</span> },
          { key: "industry", header: "行业类型", render: (r) => <span className="text-sm">{r.industry}</span> },
          {
            key: "status",
            header: "状态",
            render: (r) => (
          ...
          { key: "stores", header: "门店数量", render: (r) => <span className="text-sm">{r.storeCount}</span> },
          { key: "consults", header: "咨询量", render: (r) => <span className="text-sm">{r.consultationCount}</span> },
          { key: "tickets", header: "工单量", render: (r) => <span className="text-sm">{r.ticketCount}</span> },
          {
            key: "actions",
            header: "操作",
            render: (r) => (
      ...
      <Modal open={createModal} title="新增商家" onClose={() => setCreateModal(false)} size="md">
        <div className="space-y-3 text-sm">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">商家名称</label>
            <input
              value={form.name}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">联系电话</label>
            <input
              value={form.phone}
        ...
        {viewingMerchant && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-sm text-slate-400">商家名称</span>
                <p className="font-medium text-slate-900">{viewingMerchant.name}</p>
              </div>
              <div>
                <span className="text-sm text-slate-400">所属租户</span>
                <p className="text-slate-700">{viewingMerchant.tenantId}</p>
              </div>
              <div>
                <span className="text-sm text-slate-400">行业类型</span>
              ...
              <div>
                <span className="text-sm text-slate-400">门店数量</span>
                <p className="text-slate-700">{viewingMerchant.storeCount}</p>
              </div>
              <div>
                <span className="text-sm text-slate-400">咨询量</span>
                <p className="text-slate-700">{viewingMerchant.consultationCount}</p>
              </div>
              <div>
                <span className="text-sm text-slate-400">工单量</span>
