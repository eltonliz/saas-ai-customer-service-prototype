    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">平台角色权限</h2>
            <p className="text-sm text-slate-500 mt-1">管理平台级角色定义、成员分配及跨租户功能权限矩阵</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">角色名称</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">成员数</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">权限范围</th>
          ...
          <tbody>
            {roles.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 hover:bg-slate-50" style={{ minHeight: 48 }}>
                ...
                <td className="px-5 py-3 text-sm text-slate-600">{r.members}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{r.permissions}</td>
                ...
                <td className="px-5 py-3">
                  <button className="text-sm text-blue-600 hover:text-blue-800 mr-3">编辑权限</button>
                  <button className="text-sm text-slate-400 hover:text-slate-600">查看成员</button>
                </td>
              </tr>
            ))}
          </tbody>
