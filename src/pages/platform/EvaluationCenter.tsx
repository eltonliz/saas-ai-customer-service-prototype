...
export default function EvaluationCenter({}: PageProps) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-900 mb-4">质检与评测中心</h2>
      ...
      <div className="mb-4 flex gap-1 rounded-xl bg-slate-100 p-1 w-fit overflow-x-auto">
        {([
          ...
          <button key={t.id} type="button" onClick={() => setTab(t.id)} className={`flex items-center gap-1 rounded-lg px-4 py-2 text-sm font-medium transition-colors ${tab === t.id ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>{t.icon}{t.label}</button>
        ))}
      </div>

      {tab === "ai" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><CheckCircle size={16} className="text-blue-500" />AI回答抽检记录</h3>
          <DataTable
            data={aiSamples}
            rowKey={(r) => r.id}
            onRowClick={(r) => setSelectedId(r.id)}
            columns={[
              { key: "question", header: "用户问题", render: (r) => <span className="text-sm">{r.question.slice(0, 25)}...</span> },
              { key: "intent", header: "意图", render: (r) => <span className="text-sm">{r.intent}</span> },
              { key: "correct", header: "正确性", render: (r) => r.correct ? <span className="text-sm text-emerald-600 font-medium">正确</span> : <span className="text-sm text-rose-600 font-medium">有误</span> },
              { key: "score", header: "评分", render: (r) => <span className={`text-sm font-mono ${r.score >= 80 ? "text-emerald-600" : "text-amber-600"}`}>{r.score}</span> },
              { key: "annotator", header: "标注人", render: (r) => <span className="text-sm">{r.annotator}</span> },
            ]}
          />
        </div>
      )}
      ...
      {tab === "rag" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Search size={16} className="text-indigo-500" />RAG评测集</h3>
          <p className="text-sm text-slate-500 mb-3">用于评估检索增强生成质量的评测集合，包含30个测试用例，覆盖5个业务线。</p>
      ...
      {tab === "faq" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><BookOpen size={16} className="text-emerald-500" />FAQ匹配评测集</h3>
          <p className="text-sm text-slate-500 mb-3">评估FAQ精确匹配和语义匹配效果的评测集合，共50个测试用例。</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
      ...
      {tab === "tool" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Wrench size={16} className="text-orange-500" />工具调用评测集</h3>
          <p className="text-sm text-slate-500 mb-3">评估AI正确调用工具接口的能力，共20个场景覆盖库存查询/物流查询/订单查询/风控检查。</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
      ...
      {tab === "health" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><Shield size={16} className="text-rose-500" />大健康合规评测集</h3>
          <p className="text-sm text-slate-500 mb-3">针对大健康场景的专项评测，验证AI在疾病诊断、治疗效果、用药建议等高风险场景下的合规表现。</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
      ...
      {tab === "errors" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2"><XCircle size={16} className="text-rose-500" />错误样本库</h3>
          <DataTable
            data={errorSamples}
            rowKey={(r) => r.id}
            onRowClick={(r) => setSelectedId(r.id)}
            columns={[
              { key: "question", header: "用户问题", render: (r) => <span className="text-sm">{r.question.slice(0, 25)}...</span> },
              { key: "errorType", header: "错误类型", render: (r) => <span className="text-sm text-rose-600 font-medium">{r.errorType}</span> },
      ...
      <Drawer open={selectedId !== null} title="评测详情" onClose={() => setSelectedId(null)}>
        {sample && (
          <div className="space-y-4 text-sm">
            ...
            <div>
              <span className="text-sm text-slate-400">AI回答</span>
              <div className="mt-1 rounded-lg bg-slate-50 border border-slate-100 p-3 text-sm">{sample.aiAnswer}</div>
            </div>
            {errorDetail ? (
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-3">
                  <div><span className="text-sm text-slate-400">错误类型</span><p className="text-rose-600 font-medium">{errorDetail.errorType}</p></div>
                  <div><span className="text-sm text-slate-400">严重程度</span><StatusBadge status={errorDetail.severity} /></div>
                </div>
                <div><span className="text-sm text-slate-400">知识缺口关联</span><p>{errorDetail.knowledgeGap}</p></div>
                <div><span className="text-sm text-slate-400">处理状态</span><StatusBadge status={errorDetail.status} /></div>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <div><span className="text-sm text-slate-400">标注人</span><p>{sample.annotator}</p></div>
                <div><span className="text-sm text-slate-400">评分</span><p className="font-mono text-lg font-bold">{sample.score}</p></div>
              </div>
            )}
          </div>
        )}
