    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">数据保留策略</h2>
            <p className="text-sm text-slate-500 mt-1">管理平台各数据类型的保留周期、归档规则及合规要求</p>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">策略名称</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">数据类型</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">保留天数</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">状态</th>
              <th className="text-left px-5 py-3 text-sm font-semibold text-slate-600">更新时间</th>
          ...
          <tbody>
            {policies.map((p) => (
              <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50" style={{ minHeight: 48 }}>
                ...
                <td className="px-5 py-3 text-sm text-slate-600">{p.dataType}</td>
                <td className="px-5 py-3 text-sm text-slate-600">{p.retentionDays === 0 ? "永久" : `${p.retentionDays}天`}</td>
                ...
                <td className="px-5 py-3 text-sm text-slate-600">{p.updatedAt}</td>
                <td className="px-5 py-3">
