...
import { Settings, Route, BarChart3, Plus, Pencil, Trash2 } from "lucide-react";
...
export default function ModelConfig({}: PageProps) {
  const [providers, setProviders] = useState<Provider[]>([
    {
      ...
      temp: 0.7, maxTokens: 4096, timeout: 10000, fallback: "Claude Sonnet 4.6",
  ...
  const [editForm, setEditForm] = useState({ temp: 0.7, maxTokens: 4096, timeout: 10000, fallback: "Claude Sonnet 4.6" });
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-slate-900 mb-4">模型配置</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          {providers.map((p) => (
            <div key={p.id} className="rounded-lg border border-slate-100 bg-slate-50 p-3 mb-2">
              ...
              <p className="text-sm text-slate-500">模型: {p.models.join("、")}</p>
              <p className="text-sm text-slate-500">温度: {p.temp} · 最大输出: {p.maxTokens} · 超时: {p.timeout}ms · 降级: {p.fallback}</p>
              <div className="flex gap-2 mt-2">
                ...
                <button
                  ...
                  className="flex items-center gap-0.5 rounded-lg border border-slate-200 px-3 py-1 text-sm text-slate-500 hover:bg-slate-100"
                >
                  <Pencil size={12} />编辑
                </button>
                <button
                  ...
                  className="flex items-center gap-0.5 rounded-lg border border-red-200 px-3 py-1 text-sm text-red-500 hover:bg-red-50"
                >
                  <Trash2 size={12} />删除
                </button>
              </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="space-y-2">
            {routes.map((r) => (
              <div key={r.scene} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-0.5 rounded border border-slate-200 px-1.5 py-0.5 text-sm text-slate-400 hover:text-slate-600 hover:bg-slate-50"
                  >
                    <Pencil size={10} />
                  </button>
                  <button
                    ...
                    className="flex items-center gap-0.5 rounded border border-red-200 px-1.5 py-0.5 text-sm text-red-400 hover:text-red-600 hover:bg-red-50"
                  >
                    <Trash2 size={10} />
                  </button>
                </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <DataTable
          ...
          rowKey={(r) => r.id}
          columns={[
            { key: "scene", header: "场景", render: (r) => <span className="text-sm">{r.scene}</span> },
            { key: "model", header: "模型", render: (r) => <span className="text-sm">{r.model}</span> },
            { key: "status", header: "状态", render: (r) => <StatusBadge status={r.status} /> },
            { key: "latency", header: "延迟", render: (r) => <span className="text-sm">{r.latency}ms</span> },
            { key: "token", header: "Token", render: (r) => <span className="text-sm">{r.tokenCost}</span> },
            { key: "cost", header: "费用", render: (r) => <span className="text-sm">${r.cost.toFixed(3)}</span> },
            { key: "time", header: "时间", render: (r) => <span className="text-sm text-slate-400">{r.createdAt}</span> },
          ]}
        />
      </div>
      ...
      <Modal open={editModal} title="编辑模型参数" onClose={() => setEditModal(false)} size="md">
        <div className="space-y-3 text-sm">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">温度</label>
            <input
              ...
              className="w-full"
            />
            <span className="text-sm text-slate-400">{editForm.temp}</span>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">最大输出长度</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">超时时间 (ms)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">降级模型</label>
            <select
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditModal(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={() => setEditModal(false)} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
      ...
      <Modal open={createProviderModal} title="新增供应商" onClose={() => setCreateProviderModal(false)} size="md">
        <div className="space-y-3 text-sm">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">供应商名称</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">API Endpoint</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">模型列表（逗号分隔）</label>
            <input
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setCreateProviderModal(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleCreateProvider} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">创建</button>
        </div>
      </Modal>
      ...
      <Modal open={editProviderModal} title="编辑供应商" onClose={() => setEditProviderModal(false)} size="md">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditProviderModal(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleEditProvider} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
      ...
      <Modal open={createRouteModal} title="新增路由" onClose={() => setCreateRouteModal(false)} size="md">
        <div className="space-y-3 text-sm">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">业务场景</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">模型</label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">优先级</label>
            <select
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setCreateRouteModal(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleCreateRoute} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">创建</button>
        </div>
      </Modal>
      ...
      <Modal open={editRouteModal} title="编辑路由" onClose={() => setEditRouteModal(false)} size="md">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setEditRouteModal(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
          <button type="button" onClick={handleEditRoute} className="rounded-lg bg-blue-600 px-4 py-2 text-sm text-white">保存</button>
        </div>
      </Modal>
    </div>
  );
