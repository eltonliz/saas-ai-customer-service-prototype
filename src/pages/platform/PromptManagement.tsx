  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold text-slate-900">全局Prompt管理</h2>
        <button
          type="button"
      ...
      <DataTable
        ...
        columns={[
          { key: "name", header: "名称", render: (r) => <span className="font-medium text-sm">{r.name}</span> },
          { key: "type", header: "类型", render: (r) => <span className="text-sm">{r.type}</span> },
          { key: "version", header: "版本", render: (r) => <span className="text-sm font-mono">{r.version}</span> },
          { key: "status", header: "状态", render: (r) => <StatusBadge status={r.status} /> },
          { key: "owner", header: "负责人", render: (r) => <span className="text-sm">{r.owner}</span> },
          { key: "updatedAt", header: "更新时间", render: (r) => <span className="text-sm">{r.updatedAt}</span> },
          {
            key: "actions",
            header: "操作",
            render: (r) => (
              <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                {r.status !== "灰度中" && r.status !== "已回滚" && (
                  <button
                    ...
                    className="flex items-center gap-0.5 rounded-md border border-amber-200 bg-amber-50 px-2 py-1 text-sm text-amber-600 hover:bg-amber-100"
                  >
                    <Play size={12} />灰度
                  </button>
                )}
                {r.status === "灰度中" && (
                  <button
                    ...
                    className="flex items-center gap-0.5 rounded-md border border-red-200 bg-red-50 px-2 py-1 text-sm text-red-600 hover:bg-red-100"
                  >
        ...
        {detail && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-3">
              <div><span className="text-sm text-slate-400">名称</span><p className="font-medium">{detail.name}</p></div>
              <div><span className="text-sm text-slate-400">类型</span><p>{detail.type}</p></div>
              <div><span className="text-sm text-slate-400">版本</span><p className="font-mono">{detail.version}</p></div>
              <div><span className="text-sm text-slate-400">状态</span><StatusBadge status={detail.status} /></div>
              <div><span className="text-sm text-slate-400">负责人</span><p>{detail.owner}</p></div>
              <div><span className="text-sm text-slate-400">更新时间</span><p>{detail.updatedAt}</p></div>
            </div>
            <div>
              <span className="text-sm text-slate-400">Prompt内容</span>
              <div className="mt-1 rounded-xl bg-slate-900 p-4 text-sm text-slate-300 font-mono leading-relaxed whitespace-pre-wrap">
                {detail.content ?? "暂无内容"}
              </div>
            </div>
            {detail.grayRange && (
              <div><span className="text-sm text-slate-400">灰度范围</span><p className="text-amber-600">{detail.grayRange}</p></div>
            )}
          </div>
        )}
      </Drawer>
      ...
      <Modal open={modalNew} title="新建Prompt版本" onClose={() => setModalNew(false)} size="lg">
        <div className="space-y-3 text-sm">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">版本号</label>
            <input
              value={form.version}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1">Prompt内容</label>
            <textarea
              value={form.content}
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setModalNew(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
      ...
      <Modal open={modalEdit} title="编辑Prompt" onClose={() => setModalEdit(false)} size="lg">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button type="button" onClick={() => setModalEdit(false)} className="rounded-lg border px-4 py-2 text-sm">取消</button>
