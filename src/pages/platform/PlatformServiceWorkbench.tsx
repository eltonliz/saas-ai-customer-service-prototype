...
export default function PlatformServiceWorkbench({}: PageProps) {
  const allReqs = reqs.PlatformServiceWorkbench.flatMap(r => r.reqs);
    ...
    <div className="relative">
      ...
      <div className="mb-4 flex gap-1 rounded-xl bg-slate-100 p-1 w-fit">
        {([
          ...
          <button key={t.id} type="button" onClick={() => setTab(t.id)} className={`rounded-lg px-4 py-2 text-base font-medium transition-colors ${tab === t.id ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>{t.label}</button>
        ))}
      </div>
        ...
        <div className="flex gap-5 h-[calc(100vh-320px)] min-h-[500px]">
          <div className="w-80 flex-shrink-0 rounded-2xl border border-slate-200 bg-white overflow-y-auto">
            <div className="p-3 border-b border-slate-100">
              <span className="text-base font-semibold text-slate-500">平台兜底会话队列</span>
            </div>
            {convState.map((c) => (
              <div
                key={c.id}
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-base font-medium text-slate-700">{c.title.slice(0, 18)}</span>
                  <StatusBadge status={c.priority} />
                </div>
                <div className="flex items-center gap-2 text-base text-slate-400 mb-1">
                  <Users size={10} />{c.user}
                </div>
                <div className="flex items-center gap-1">
                  <StatusBadge status={c.status} />
                  <span className="text-base text-slate-400">{c.tenant}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {activeConv ? (
              <>
                <div className="p-3 border-b border-slate-100 flex items-center justify-between">
                  <div>
                    <span className="text-base font-semibold text-slate-700">{activeConv.title}</span>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-base text-slate-400">{activeConv.user}</span>
                      <span className="text-base text-slate-400">·</span>
                      <span className="text-base text-slate-400">{activeConv.tenant}</span>
                    </div>
                  </div>
                ...
                <div className="p-3 border-t border-slate-100">
                  <div className="flex gap-2 mb-2">
                    {activeConv.status !== "人工接待中" && (
                      <button type="button" onClick={() => handleAccept(activeConv.id)} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-base text-white hover:bg-blue-700"><CheckCircle size={12} />接入</button>
                    )}
                    {activeConv.status === "人工接待中" && (
                      <>
                        <button type="button" onClick={() => setCreateTicketOpen(true)} className="flex items-center gap-1 rounded-lg bg-slate-100 px-3 py-1.5 text-base text-slate-600 hover:bg-slate-200"><Ticket size={12} />建工单</button>
                        <button type="button" onClick={() => handleClose(activeConv.id)} className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-base text-slate-500 hover:bg-slate-50"><XCircle size={12} />结束</button>
                      </>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} placeholder="输入回复..." className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-base outline-none focus:border-blue-400" />
              ...
              <div className="flex-1 flex items-center justify-center text-base text-slate-400">
                请从左侧选择一个会话
              </div>
            )}
          </div>
          ...
          {activeConv && (
            <div className="w-60 flex-shrink-0 rounded-2xl border border-slate-200 bg-white p-3 overflow-y-auto">
              <h4 className="text-base font-semibold text-slate-500 mb-2">会话信息</h4>
      ...
      {tab === "tickets" && (
        <div className="rounded-2xl border border-slate-200 bg-white p-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-base font-semibold text-slate-700">平台工单列表</h3>
            <button type="button" onClick={() => setCreateTicketOpen(true)} className="flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-base text-white hover:bg-blue-700"><Ticket size={12} />新建工单</button>
          </div>
              ...
              <div key={t.id} className="flex items-center justify-between rounded-lg border border-slate-100 p-3 hover:bg-slate-50 cursor-pointer">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-base font-medium text-slate-700">{t.title}</span>
                ...
                <span className="text-base text-slate-400">{t.createdAt}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <Modal open={createTicketOpen} title="新建工单" onClose={() => setCreateTicketOpen(false)}>
        <div className="space-y-4">
          <div>
            <label className="text-base font-medium text-slate-700">工单标题</label>
            <input value={newTicket.title} onChange={(e) => setNewTicket((p) => ({ ...p, title: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-base outline-none focus:border-blue-400" placeholder="输入工单标题" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-base font-medium text-slate-700">类型</label>
              <select value={newTicket.type} onChange={(e) => setNewTicket((p) => ({ ...p, type: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-base outline-none">
                <option>系统故障</option><option>跨商家争议</option><option>平台投诉</option><option>审核工单</option><option>其他</option>
              </select>
            </div>
            <div>
              <label className="text-base font-medium text-slate-700">优先级</label>
              <select value={newTicket.priority} onChange={(e) => setNewTicket((p) => ({ ...p, priority: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-base outline-none">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-base font-medium text-slate-700">负责人</label>
              <input value={newTicket.owner} onChange={(e) => setNewTicket((p) => ({ ...p, owner: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-base outline-none focus:border-blue-400" placeholder="负责人" />
            </div>
            <div>
              <label className="text-base font-medium text-slate-700">SLA</label>
              <select value={newTicket.sla} onChange={(e) => setNewTicket((p) => ({ ...p, sla: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-base outline-none">
                <option>2小时</option><option>4小时</option><option>8小时</option><option>24小时</option><option>48小时</option>
              </select>
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setCreateTicketOpen(false)} className="rounded-lg border border-slate-200 px-4 py-2 text-base text-slate-600 hover:bg-slate-50">取消</button>
            ...
            }} className="rounded-lg bg-blue-600 px-4 py-2 text-base text-white hover:bg-blue-700">确认创建</button>
          </div>
        </div>
      </Modal>
    </div>
