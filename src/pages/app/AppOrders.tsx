...
export default function AppOrders({ context, goPage }: PageProps) {
  ...
  return (
    <div className="p-4">
      <h2 className="text-xl font-bold text-slate-900 mb-4">我的订单</h2>
      {!selected ? (
            ...
            <button key={o.id} type="button" onClick={() => setSelectedId(o.id)} className="w-full rounded-2xl bg-white p-4 shadow-sm border border-slate-100 text-left transition-shadow hover:shadow-md">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-medium text-slate-800 block truncate">{o.productName}</span>
                  <p className="text-sm text-slate-400 mt-0.5">{o.spec} · ￥{o.amount}</p>
                </div>
                <StatusBadge status={o.status} />
              </div>
              <p className="text-sm text-slate-400">{o.createdAt}</p>
            </button>
          ))}
        </div>
      ) : (
        <div>
          <button type="button" onClick={() => setSelectedId(null)} className="mb-4 text-sm text-blue-600 hover:text-blue-700">&larr; 返回列表</button>
          <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-slate-900">{selected.productName}</h3>
            ...
            <div className="space-y-3 text-sm text-slate-600 mb-4">
              ...
              <div className="flex justify-between"><span className="text-slate-400">收货信息</span><span className="text-sm">{selected.receiverInfo}</span></div>
            ...
            <div className="mb-4">
              <p className="text-sm font-medium text-slate-700 mb-3">物流详情</p>
              <Timeline items={selected.logisticsTimeline.map((t) => ({ time: "", title: t, state: "done" as const }))} />
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => goPage?.("after-sales", { afterSaleOrderId: selected.id })} className="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm font-medium text-amber-700 hover:bg-amber-100">
                <Wrench size={16} />申请售后
              </button>
              <button type="button" onClick={() => goPage?.("ai-service", { chatPrompt: `咨询订单 ${selected.id}` })} className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-medium text-blue-700 hover:bg-blue-100">
